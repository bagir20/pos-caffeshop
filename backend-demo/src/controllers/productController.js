...
exports.createProduct = async (req, res) => {
  try {
    ...
    res.json({ message: 'Product created', product: withCategory(data[0]) });
  } catch (error) {
    console.error(error);
...
exports.updateProduct = async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from('products')
      .update({ name, price: Number(price), category_id: Number(category_id), image_url })
      .eq('id', id)
      .select();

    if (error) throw error;

    res.json({ message: 'Product updated', product: withCategory(data[0]) });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Server error' });
  }
...
exports.toggleAvailability = async (req, res) => {
  try {
    ...
    res.json({ id, name: data.name, is_available: !data.is_available });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Server error' });
  }
